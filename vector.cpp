...
public:
    ...
    Vector(T* arr_0, T* arr_n); //constructor sao chep mang
    Vector(typename Vector<T>::iterator first, typename Vector<T>::iterator last); // constructor sao chep tu day iterator
    Vector(const Vector<T>& V); //constructor sao chep
    Vector<T>& operator=(const Vector<T>& V); //toan tu gan sao chep    
    ...
    /*-----MODIFIERS-----*/
    void clear(); //xoa tat ca phan tu trong vector(khong thay doi dung luong)
    void push_back(const T& val);
    void pop_back();
    void assign(size_t s, const T& value); // gan s phan tu voi gia tri mac dinh value
    void assign(T* first, T* last);
    void assign(typename Vector<T>::iterator first, typename Vector<T>::iterator last);
    void assign(initializer_list<T> ilist);
    iterator insert(typename Vector<T>::iterator position, const T& value); 
    void insert(typename Vector<T>::iterator position, size_t n, const T& value); //chen n phan tu voi gia tri value
    ...
    iterator erase(typename Vector<T>::iterator start_position, typename Vector<T>::iterator end_position);  //xoa phan tu tu start -> end
    ...
    void shrink_to_fit();
private:
    size_t Size; //so luong phan tu
    size_t Space; //dung luong(gom ca phan tu va khong gian trong)
    T* Elements; //con tro toi phan tu dau tien cua vectorvector
};
...
//iterator
template <typename T>
inline typename Vector<T>::iterator Vector<T>::begin() const {  //typename:Vector<t>::iterator 
...
inline typename Vector<T>::iterator Vector<T>::insert(typename Vector<T>::iterator position, const T& value) {
    ...
    *(begin() + temp) = value;
    return begin() + temp;
} // tra ve con tro tro toi phan tu moi chen
template <typename T>
